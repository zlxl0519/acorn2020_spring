...
public class FriendsController {
	...
	public String friends(HttpServletRequest request) {
		...
		list.add("해골");
		list.add("원숭이");
		//view 페이지에 전달할 모델을 request 영역에 답고
		request.setAttribute("list", list);
		// /WEB-INF/views/friends.jsp
	...
	public String delete() {
		...
		//삭제후 친구 목록 보기로 리다일렉트 이동 // context 경로를 안써도 된다.
		return "redirect:/friends.do";//redirect: 으로 시작해야 redirect 안붙이면 forward 이다.
	}
	...
	public ModelAndView delete2(ModelAndView mView) {
		//가상의 삭제 비즈니스 로직 수행
		System.out.println("친구 정보를 삭제 합니다.");
		//ModelAndView 객체에 리다일레트 이동 정보를 담아서
